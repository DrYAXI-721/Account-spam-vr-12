const express = require("express");
const app = express();
const dreams = [
  "Find and count some sheep",
  "Climb a really tall mountain",
  "Wash the dishes"
];
app.use(express.static("public"));
app.get("/", (request, response) => {
  response.sendFile(__dirname + "/views/index.html");
});
app.get("/dreams", (request, response) => {
  response.json(dreams);
});
const listener = app.listen(process.env.PORT, () => {
  console.log("Your app is listening on port " + listener.address().port);
});
const Discord = require("discord.js");
const mars = new Discord.Client();
const mars2 = new Discord.Client();
const mars3 = new Discord.Client();
const mars4 = new Discord.Client();
const mars5 = new Discord.Client();
const mars6 = new Discord.Client();
const mars7 = new Discord.Client();
const mars8 = new Discord.Client();
const mars9 = new Discord.Client();
const mars10 = new Discord.Client();
const mars11 = new Discord.Client();
const mars12 = new Discord.Client();
const mars13 = new Discord.Client();
const mars14 = new Discord.Client();
const mars15 = new Discord.Client();
const mars16 = new Discord.Client();
const mars17 = new Discord.Client();
const mars18 = new Discord.Client();
const mars19 = new Discord.Client();
const mars20 = new Discord.Client();
const mars21 = new Discord.Client();
const mars22 = new Discord.Client();
const mars23 = new Discord.Client();
const mars24 = new Discord.Client();
const mars25 = new Discord.Client();
const mars26 = new Discord.Client();
const mars27 = new Discord.Client();
const mars28 = new Discord.Client();
const mars29 = new Discord.Client();
const mars30 = new Discord.Client();
const mars31 = new Discord.Client();
const mars32 = new Discord.Client();
const mars33 = new Discord.Client();
const mars34 = new Discord.Client();
const mars35 = new Discord.Client();
const mars36 = new Discord.Client();
const mars37 = new Discord.Client();
const mars38 = new Discord.Client();
const mars39 = new Discord.Client();
const mars40 = new Discord.Client();
const mars41 = new Discord.Client();
const mars42 = new Discord.Client();
const mars43 = new Discord.Client();
const mars44 = new Discord.Client();
const mars45 = new Discord.Client();
const mars46 = new Discord.Client();
const mars47 = new Discord.Client();
const mars48 = new Discord.Client();
const mars49 = new Discord.Client();
const mars50 = new Discord.Client();
const mars51 = new Discord.Client();
const mars52 = new Discord.Client();
const mars53 = new Discord.Client();
const mars54 = new Discord.Client();
const mars55 = new Discord.Client();
const mars56 = new Discord.Client();
const mars57 = new Discord.Client();
const mars58 = new Discord.Client();
const mars59 = new Discord.Client();
const mars60 = new Discord.Client();
const mars61 = new Discord.Client();
const mars62 = new Discord.Client();
const mars63 = new Discord.Client();
const mars64 = new Discord.Client();
const mars65 = new Discord.Client();
const mars66 = new Discord.Client();
const mars67 = new Discord.Client();
const mars68 = new Discord.Client();
const mars69 = new Discord.Client();
const mars70 = new Discord.Client();
const mars71 = new Discord.Client();
const mars72 = new Discord.Client();
const mars73 = new Discord.Client();
const mars74 = new Discord.Client();
const mars75 = new Discord.Client();
const mars76 = new Discord.Client();
const mars77 = new Discord.Client();
const mars78 = new Discord.Client();
const mars79 = new Discord.Client();
const mars80 = new Discord.Client();
const mars81 = new Discord.Client();
const mars82 = new Discord.Client();
const mars83 = new Discord.Client();
const mars84 = new Discord.Client();
const mars85 = new Discord.Client();
const mars86 = new Discord.Client();
const mars87 = new Discord.Client();
const mars88 = new Discord.Client();
const mars89 = new Discord.Client();
const mars90 = new Discord.Client();
const mars91 = new Discord.Client();
const mars92 = new Discord.Client();
const mars93 = new Discord.Client();
const mars94 = new Discord.Client();
const mars95 = new Discord.Client();
const mars96 = new Discord.Client();
const mars97 = new Discord.Client();
const mars98 = new Discord.Client();
const mars99 = new Discord.Client();
const mars100 = new Discord.Client();
//////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////
mars.on("ready", ready => {
  mars.user.setActivity("END WEB ");
  console.log("1");
});
mars.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
//////////////////////////////////////////////////////////////////////////////////////////////
mars2.on("ready", ready => {
  mars2.user.setActivity("END WEB ");
  console.log("2");
});

mars2.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars2.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars2.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars2.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars2.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars2.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars2.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars2.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars2.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars2.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
//////////////////////////////////////////////////////////////
mars3.on("ready", ready => {
  mars3.user.setActivity("END web ");
  console.log("3");
});

mars3.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars3.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars3.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars3.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars3.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars3.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars3.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars3.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars3.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars3.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
//////////////////////////////////////////////////////
mars4.on("ready", ready => {
  mars4.user.setActivity("END WEB ");
  console.log("4");
});

mars4.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars4.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars4.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars4.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars4.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars4.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars4.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars4.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars4.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars4.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////
mars5.on("ready", ready => {
  mars5.user.setActivity("END WEB ");
  console.log("5");
});

mars5.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars5.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars5.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars5.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars5.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars5.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars5.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars5.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars5.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars5.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////////////////////////
mars6.on("ready", ready => {
  mars6.user.setActivity("END WEB");
  console.log("6");
});

mars6.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars6.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars6.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars6.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars6.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars6.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars6.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars6.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars6.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars6.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
//////////////////////////////////////////
mars7.on("ready", ready => {
  mars7.user.setActivity("END WEB");
  console.log("7");
});

mars7.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars7.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars7.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars7.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars7.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars7.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars7.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars7.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars7.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars7.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////////////////
mars8.on("ready", ready => {
  mars8.user.setActivity("END WEB ");
  console.log("8");
});

mars8.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars8.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars8.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars8.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars8.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars8.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars8.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars8.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars8.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars8.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
/////////////////////////////////////////////////
mars9.on("ready", ready => {
  mars9.user.setActivity("END WEB");
  console.log("9");
});

mars9.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars9.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars9.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars9.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars9.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars9.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars9.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars9.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars9.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars9.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars10.on("ready", ready => {
  mars10.user.setActivity("END WEB");
  console.log("10");
});

mars10.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars10.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars10.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars10.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars10.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars10.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars10.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars10.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars10.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars10.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars11.on("ready", ready => {
  mars11.user.setActivity("END WEB ");
  console.log("11");
});

mars11.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars11.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars11.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars11.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars11.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars11.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars11.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars11.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars11.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars11.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars12.on("ready", ready => {
  mars12.user.setActivity("END WEB");
  console.log("12");
});

mars12.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars12.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars12.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars12.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars12.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars12.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars12.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars12.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars12.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars12.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars13.on("ready", ready => {
  mars13.user.setActivity("END WEB ");
  console.log("13");
});

mars13.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars13.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars13.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars13.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars13.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars13.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars13.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars13.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars13.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars13.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars14.on("ready", ready => {
  mars14.user.setActivity("END WEB");
  console.log("14");
});

mars14.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars14.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars14.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars14.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars14.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars14.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars14.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars14.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars14.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars14.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars15.on("ready", ready => {
  mars15.user.setActivity("END WEB");
  console.log("15");
});

mars15.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars15.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars15.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars15.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars15.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars15.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars15.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars15.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars15.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars15.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars15.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars16.on("ready", ready => {
  mars16.user.setActivity("END WEB");
  console.log("16");
});

mars16.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars16.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars16.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars16.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars16.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars16.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars16.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars16.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars16.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars16.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars17.on("ready", ready => {
  mars17.user.setActivity("END WEB");
  console.log("17");
});

mars17.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars17.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars17.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars17.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars17.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars17.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars17.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars17.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars17.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars17.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars18.on("ready", ready => {
  mars18.user.setActivity("END WEB");
  console.log("18");
});

mars18.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars18.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars18.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars18.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars18.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars18.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars18.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars18.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars18.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars18.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars19.on("ready", ready => {
  mars19.user.setActivity("END WEB");
  console.log("19");
});

mars19.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars19.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars19.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars19.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars19.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars19.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars19.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars19.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars19.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars19.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars20.on("ready", ready => {
  mars20.user.setActivity("END WEB");
  console.log("20");
});

mars20.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars20.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars20.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars20.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars20.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars20.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars20.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars20.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars20.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars20.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars21.on("ready", ready => {
  mars21.user.setActivity("END WEB");
  console.log("21");
});

mars21.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars21.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars21.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars21.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars21.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars21.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars21.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars21.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars21.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars21.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars22.on("ready", ready => {
  mars22.user.setActivity("END WEB");
  console.log("22");
});

mars22.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars22.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars22.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars22.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars22.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars22.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars22.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars22.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars22.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars22.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars23.on("ready", ready => {
  mars23.user.setActivity("END WEB");
  console.log("23");
});

mars23.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars23.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars23.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars23.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars23.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars23.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars23.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars23.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars23.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars23.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars24.on("ready", ready => {
  mars24.user.setActivity("END WEB");
  console.log("24");
});

mars24.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars24.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars24.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars24.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars24.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars24.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars24.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars24.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars24.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars24.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////
mars25.on("ready", ready => {
  mars25.user.setActivity("END WEB");
  console.log("25");
});

mars25.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars25.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars25.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars25.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars25.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars25.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars25.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars25.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars25.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars25.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
/////////////////////////////////////////////
mars26.on("ready", ready => {
  mars26.user.setActivity("cEND WEB");
  console.log("26");
});
mars26.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars26.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars26.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars26.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars26.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars26.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars26.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars26.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars26.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars26.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////////
mars27.on("ready", ready => {
  mars27.user.setActivity("END WEB");
  console.log("27");
});
mars27.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars27.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars27.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars27.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars27.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars27.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars27.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars27.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars27.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars27.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars27.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars27.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
//////////////////////////////////
mars28.on("ready", ready => {
  mars28.user.setActivity("END WEB");
  console.log("29");
});
mars28.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars28.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars28.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars28.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars28.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars28.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars28.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars28.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////
mars29.on("ready", ready => {
  mars29.user.setActivity("END WEB");
  console.log("29");
});
mars29.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars29.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars29.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars29.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars29.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars29.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars29.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars29.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
//////////////////////////////////////
mars30.on("ready", ready => {
  mars30.user.setActivity("END WEB");
  console.log("30");
});
mars30.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars30.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars30.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars30.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars30.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars30.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars30.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars30.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars30.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars30.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
//////////////////////////////////////////////////////////
mars31.on("ready", ready => {
  mars31.user.setActivity("END WEB");
  console.log("31");
});

mars31.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars31.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars31.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars31.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars31.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars31.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars31.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars31.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars31.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars31.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
/////////////////////////////////////////////////////////////
mars32.on("ready", ready => {
  mars32.user.setActivity("END WEB");
  console.log("32");
});

mars32.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars32.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars32.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars32.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars32.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars32.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars32.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars32.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars32.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars32.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////////////////////////////////
mars33.on("ready", ready => {
  mars33.user.setActivity("END WEB");
  console.log("33");
});

mars33.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars33.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars33.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars33.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars33.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars33.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars33.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars33.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars33.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars33.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
//////////////////////////////////////////////////////
mars34.on("ready", ready => {
  mars34.user.setActivity("END WEB");
  console.log("34");
});

mars34.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars34.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars34.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars34.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars34.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars34.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars34.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars34.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars34.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars34.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
/////////////////////////////////////////////////////////
mars35.on("ready", ready => {
  mars35.user.setActivity("END WEB");
  console.log("35");
});

mars35.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars35.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars35.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars35.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars35.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars35.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars35.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars35.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars35.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars35.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
}); ///////////////////////////////////////////
mars36.on("ready", ready => {
  mars36.user.setActivity("END WEB");
  console.log("36");
});

mars36.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars36.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars36.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars36.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars36.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars36.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars36.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars36.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars36.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars36.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////
mars37.on("ready", ready => {
  mars37.user.setActivity("END WEB");
  console.log("37");
});

mars37.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars37.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});

mars37.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});

mars37.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});

mars37.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});

mars37.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});

mars37.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});

mars37.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});

mars37.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////////////////
mars38.on("ready", ready => {
  mars38.user.setActivity("END WEB");
  console.log("38");
});

mars38.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars38.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars38.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars38.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars38.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars38.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars38.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars38.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars38.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars38.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////////////////
mars39.on("ready", ready => {
  mars39.user.setActivity("END WEB");
  console.log("39");
});

mars39.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars39.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars39.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars39.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars39.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars39.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars39.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars39.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars39.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars39.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});

///////////////////////////////////////////////////////////
mars40.on("ready", ready => {
  mars40.user.setActivity("END WEB");
  console.log("40");
});

mars40.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars40.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars40.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars40.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars40.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars40.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars40.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars40.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars40.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars40.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars41.on("ready", ready => {
  mars41.user.setActivity("END WEB");
  console.log("41");
});

mars41.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars41.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars41.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars41.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars41.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars41.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars41.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars41.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars41.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars41.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars42.on("ready", ready => {
  mars42.user.setActivity("END WEB");
  console.log("42");
});

mars42.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars42.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars42.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars42.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars42.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars42.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars42.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars42.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars42.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars42.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
//////////////////////////////////////////////////////
mars43.on("ready", ready => {
  mars43.user.setActivity("END WEB");
  console.log("43");
});

mars43.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars43.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars43.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars43.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars43.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars43.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars43.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars43.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars43.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars43.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
//////////////////////////////////////////////////////
mars44.on("ready", ready => {
  mars44.user.setActivity("END WEB");
  console.log("44");
});

mars44.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars44.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars44.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars44.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars44.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars44.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars44.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars44.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars44.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars44.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
//////////////////////////////////////////////////
mars45.on("ready", ready => {
  mars45.user.setActivity("END WEB");
  console.log("45");
});

mars45.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars45.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars45.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars45.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars45.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars45.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars45.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars45.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars45.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars45.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
///////////////////////////////////////////////////
mars46.on("ready", ready => {
  mars46.user.setActivity("END WEB");
  console.log("46");
});

mars46.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars46.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars46.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars46.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars46.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars46.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars46.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars46.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars46.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars46.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
/////////////////////////////////////////////////
mars47.on("ready", ready => {
  mars47.user.setActivity("END WEB");
  console.log("47");
});

mars47.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars47.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars47.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars47.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars47.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars47.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars47.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars47.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars47.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars47.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////
mars48.on("ready", ready => {
  mars48.user.setActivity("END WEB");
  console.log("48");
});

mars48.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars48.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars48.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars48.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars48.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars48.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars48.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars48.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars48.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars48.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////
mars49.on("ready", ready => {
  mars49.user.setActivity("END WEB");
  console.log("49");
});

mars49.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars49.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars49.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars49.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars49.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars49.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars49.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars49.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars49.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars49.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
/////////////////////////////////////////////////////
mars50.on("ready", ready => {
  mars50.user.setActivity("END WEB");
  console.log("50");
});

mars50.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars50.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars50.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars50.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars50.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars50.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars50.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars50.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars50.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars50.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
/////////////////////////////////////////////////////
mars51.on("ready", ready => {
  mars51.user.setActivity("END WEB");
  console.log("51");
});

mars51.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars51.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars51.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars51.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars51.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars51.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars51.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars51.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars51.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars51.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});

////////////////////////////////////////////////////
mars52.on("ready", ready => {
  mars52.user.setActivity("END WEB");
  console.log("52");
});

mars52.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars52.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars52.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars52.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars52.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars52.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars52.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars52.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars52.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars52.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars53.on("ready", ready => {
  mars53.user.setActivity("END WEB");
  console.log("53");
});

mars53.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars53.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars53.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars53.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars53.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars53.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars53.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars53.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars53.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars53.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars54.on("ready", ready => {
  mars54.user.setActivity("END WEB");
  console.log("54");
});

mars54.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars54.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars54.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars54.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars54.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars54.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars54.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars54.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars54.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars54.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars55.on("ready", ready => {
  mars55.user.setActivity("END WEB");
  console.log("55");
});

mars55.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars55.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars55.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars55.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars55.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars55.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars55.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars55.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars55.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars55.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars56.on("ready", ready => {
  mars56.user.setActivity("END WEB");
  console.log("56");
});

mars56.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars56.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars56.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars56.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars56.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars56.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars56.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars56.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars56.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars56.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars57.on("ready", ready => {
  mars57.user.setActivity("END WEB");
  console.log("57");
});

mars57.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars57.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars57.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars57.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars57.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars57.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars57.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars57.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars57.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars57.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars58.on("ready", ready => {
  mars58.user.setActivity("END WEB");
  console.log("58");
});

mars58.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars58.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars58.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars58.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars58.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars58.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars58.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars58.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars58.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars58.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars59.on("ready", ready => {
  mars59.user.setActivity("END WEB");
  console.log("59");
});

mars59.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars59.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars59.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars59.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars59.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars59.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars59.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars59.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars59.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars59.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars60.on("ready", ready => {
  mars60.user.setActivity("END WEB");
  console.log("60");
});

mars60.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});


mars60.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});


mars60.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});


mars60.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});


mars60.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});

mars60.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});


mars60.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});

mars60.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});


mars60.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});


mars60.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars61.on("ready", ready => {
  mars61.user.setActivity("END WEB");
  console.log("61");
});

mars61.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});


mars61.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});


mars61.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});

mars61.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars61.on("ready", ready => {
  mars61.user.setActivity("code by mars");
  console.log("61");
});

mars61.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});


mars61.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});


mars61.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});


mars61.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});


mars61.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});


mars61.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars62.on("ready", ready => {
  mars62.user.setActivity("END WEB");
  console.log("62");
});

mars62.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars62.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars62.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars62.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars62.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars62.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars62.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars62.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars62.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars62.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars63.on("ready", ready => {
  mars63.user.setActivity("END WEB");
  console.log("63");
});

mars63.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars63.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars63.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars63.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars63.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars63.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars63.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars63.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars63.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars63.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars64.on("ready", ready => {
  mars64.user.setActivity("END WEB");
  console.log("64");
});

mars64.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars64.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars64.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars64.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars64.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars64.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars64.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars64.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars64.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars64.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars65.on("ready", ready => {
  mars65.user.setActivity("END WEB");
  console.log("65");
});

mars65.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars65.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars65.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars65.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars65.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars65.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars65.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars65.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars65.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars65.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars66.on("ready", ready => {
  mars66.user.setActivity("END WEB");
  console.log("66");
});

mars66.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars66.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars66.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars66.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars66.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars66.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars66.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars66.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars66.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars66.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars67.on("ready", ready => {
  mars67.user.setActivity("END WEB");
  console.log("67");
});

mars67.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars67.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars67.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars67.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars67.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars67.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars67.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars67.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars67.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars67.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars68.on("ready", ready => {
  mars68.user.setActivity("END WEB");
  console.log("68");
});

mars68.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars68.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars68.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars68.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars68.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars68.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars68.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars68.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars68.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars68.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars69.on("ready", ready => {
  mars69.user.setActivity("END WEB");
  console.log("69");
});

mars69.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars69.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars69.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars69.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars69.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars69.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars69.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars69.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars69.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars69.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars70.on("ready", ready => {
  mars70.user.setActivity("END WEB");
  console.log("70");
});

mars70.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars70.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars70.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars70.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars70.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars70.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars70.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars70.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars70.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars70.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});

////////////////////////////////////////////////////
mars71.on("ready", ready => {
  mars71.user.setActivity("END WEB");
  console.log("71");
});

mars71.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars71.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars71.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars71.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars71.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars71.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars71.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars71.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars71.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars71.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars72.on("ready", ready => {
  mars72.user.setActivity("END WEB");
  console.log("72");
});

mars72.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars72.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars72.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars72.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars72.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars72.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars72.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars72.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars72.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars72.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars73.on("ready", ready => {
  mars73.user.setActivity("END WEB");
  console.log("73");
});

mars73.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars73.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars73.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars73.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars73.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars73.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars73.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars73.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars73.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars73.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars74.on("ready", ready => {
  mars74.user.setActivity("END WEB");
  console.log("74");
});

mars74.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars74.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars74.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars74.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars74.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars74.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars74.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars74.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars74.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars74.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars74.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars75.on("ready", ready => {
  mars75.user.setActivity("END WEB");
  console.log("75");
});

mars75.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars75.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars75.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars75.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars75.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars75.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars75.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars75.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars75.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars75.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});

////////////////////////////////////////////////////
mars76.on("ready", ready => {
  mars76.user.setActivity("END WEB");
  console.log("76");
});

mars76.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars76.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars76.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars76.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars76.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars76.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars76.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars76.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars76.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars76.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars77.on("ready", ready => {
  mars77.user.setActivity("END WEB");
  console.log("77");
});

mars77.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars77.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars77.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars77.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars77.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars77.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars77.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars77.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars77.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars77.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars78.on("ready", ready => {
  mars78.user.setActivity("END WEB");
  console.log("78");
});

mars78.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars78.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars78.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars78.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars78.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars78.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars78.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars78.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars78.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars78.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars79.on("ready", ready => {
  mars79.user.setActivity("END WEB");
  console.log("79");
});

mars79.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars79.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars79.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars79.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars79.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars79.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars79.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars79.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars79.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars79.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars80.on("ready", ready => {
  mars80.user.setActivity("END WEB");
  console.log("90");
});

mars80.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars80.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars80.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars80.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars80.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars80.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars80.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars80.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars80.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars80.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars81.on("ready", ready => {
  mars81.user.setActivity("END WEB");
  console.log("81");
});

mars81.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars81.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars81.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars81.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars81.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars81.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars81.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars81.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars81.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars81.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars82.on("ready", ready => {
  mars82.user.setActivity("END WEB");
  console.log("82");
});

mars82.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars82.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars82.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars82.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars82.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars82.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars82.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars82.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars82.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars82.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars83.on("ready", ready => {
  mars83.user.setActivity("END WEB");
  console.log("83");
});

mars83.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars83.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars83.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars83.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars83.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars83.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars83.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars83.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars83.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars83.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars84.on("ready", ready => {
  mars84.user.setActivity("END WEB");
  console.log("84");
});

mars84.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars84.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars84.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars84.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars84.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars84.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars84.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars84.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars84.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars84.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars85.on("ready", ready => {
  mars85.user.setActivity("END WEB");
  console.log("85");
});

mars85.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars85.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars85.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars85.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars85.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars85.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars85.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars85.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars85.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars85.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars86.on("ready", ready => {
  mars86.user.setActivity("END WEB");
  console.log("86");
});

mars86.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars86.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars86.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars86.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars86.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars86.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars86.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars86.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars86.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars86.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars87.on("ready", ready => {
  mars87.user.setActivity("END WEB");
  console.log("87");
});

mars87.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars87.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars87.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars87.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars87.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars87.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars87.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars87.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars87.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars87.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars88.on("ready", ready => {
  mars88.user.setActivity("code by mars");
  console.log("88");
});

mars88.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars88.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars88.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars88.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars88.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars88.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars88.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars88.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars88.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars88.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars89.on("ready", ready => {
  mars89.user.setActivity("END WEB");
  console.log("89");
});

mars89.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars89.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars89.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars89.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars89.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars89.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars89.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars89.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars89.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars89.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars90.on("ready", ready => {
  mars90.user.setActivity("END WEB");
  console.log("90");
});

mars90.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars90.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars90.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars90.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars90.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars90.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars90.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars90.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars90.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars90.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars91.on("ready", ready => {
  mars91.user.setActivity("END WEB");
  console.log("91");
});

mars91.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars91.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars91.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars91.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars91.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars91.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars91.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars91.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars91.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars91.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars92.on("ready", ready => {
  mars92.user.setActivity("code by mars");
  console.log("92");
});

mars92.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars92.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars92.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars92.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars92.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars92.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars92.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars92.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars92.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars92.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars93.on("ready", ready => {
  mars93.user.setActivity("END WEB");
  console.log("93");
});

mars93.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars93.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars93.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars93.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars93.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars93.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars93.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars93.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars93.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars93.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars94.on("ready", ready => {
  mars94.user.setActivity("END WEB");
  console.log("94");
});

mars94.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars94.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars94.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars94.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars94.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars94.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars94.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars94.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars94.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars94.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars95.on("ready", ready => {
  mars95.user.setActivity("END WEB");
  console.log("95");
});

mars95.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars95.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars95.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars95.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars95.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars95.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars95.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars95.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars95.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars95.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars96.on("ready", ready => {
  mars96.user.setActivity("END WEB");
  console.log("96");
});

mars96.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars96.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars96.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars96.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars96.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars96.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars96.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars96.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars96.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars96.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars97.on("ready", ready => {
  mars97.user.setActivity("END WEB");
  console.log("97");
});

mars97.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars97.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars97.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars97.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars97.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars97.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars97.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars97.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars97.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars97.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars98.on("ready", ready => {
  mars98.user.setActivity("END WEB");
  console.log("98");
});

mars98.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars98.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars98.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars98.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars98.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars98.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars98.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars98.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars98.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars98.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars99.on("ready", ready => {
  mars99.user.setActivity("END WEB");
  console.log("99");
});

mars99.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars99.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars99.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars99.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars99.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars99.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars99.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars99.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars99.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars99.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
////////////////////////////////////////////////////
mars100.on("ready", ready => {
  mars100.user.setActivity("END WEB");
  console.log("100");
});

mars100.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars100.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars100.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars100.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars100.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars100.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars100.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars100.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars100.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
mars100.on("message", message => {
  if (message.content === "spam") {
    message.channel.send(
      ""
    );
  }
});
//////////////////////////// voice ////////////////////////
mars.on("ready", () => {
    var join = mars.channels.get("804798316142395402"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars2.on("ready", () => {
    var join = mars2.channels.get("804798316142395402"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars3.on("ready", () => {
    var join = mars3.channels.get("806168157533634643"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars4.on("ready", () => {
    var join = mars4.channels.get("806168190957781043"); ///kaka sawt naya ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars5.on("ready", () => {
    var join = mars5.channels.get("806168236017582151"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars6.on("ready", () => {
    var join = mars6.channels.get("806168312676483153"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars7.on("ready", () => {
    var join = mars7.channels.get("806168350752243734"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars8.on("ready", () => {
    var join = mars8.channels.get("806168389818253365"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars9.on("ready", () => {
    var join = mars9.channels.get("806168442327007238"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars10.on("ready", () => {
    var join = mars10.channels.get("806168550389055508"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars11.on("ready", () => {
    var join = mars11.channels.get("806203511623647353"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars12.on("ready", () => {
    var join = mars12.channels.get("806222675302940713"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars13.on("ready", () => {
    var join = mars13.channels.get("806222716100280371"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars14.on("ready", () => {
    var join = mars14.channels.get("806222753224327228"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars15.on("ready", () => {
    var join = mars15.channels.get("806222820094509146"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars16.on("ready", () => {
    var join = mars16.channels.get("806222940563308604"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars17.on("ready", () => {
    var join = mars17.channels.get("806222970791526432"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars18.on("ready", () => {
    var join = mars18.channels.get("806223002382762024"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars19.on("ready", () => {
    var join = mars19.channels.get("806223034678509629"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars20.on("ready", () => {
    var join = mars20.channels.get("806223066064486489"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars21.on("ready", () => {
    var join = mars21.channels.get("806223100529999933"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars22.on("ready", () => {
    var join = mars22.channels.get("806223176560410670"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars23.on("ready", () => {
    var join = mars23.channels.get("806223205639520297"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars24.on("ready", () => {
    var join = mars24.channels.get("806223235497721886"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars25.on("ready", () => {
    var join = mars25.channels.get("806223269433966643"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars26.on("ready", () => {
    var join = mars26.channels.get("806223298979299368"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars27.on("ready", () => {
    var join = mars27.channels.get("806223350262530150"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars28.on("ready", () => {
    var join = mars28.channels.get("806223383532142662"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars29.on("ready", () => {
    var join = mars29.channels.get("806223414389243945"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars30.on("ready", () => {
    var join = mars30.channels.get("806223468969459722"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars31.on("ready", () => {
    var join = mars31.channels.get("806223501283426325"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars32.on("ready", () => {
    var join = mars32.channels.get("806223533440237639"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars33.on("ready", () => {
    var join = mars33.channels.get("806223561944727584"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars34.on("ready", () => {
    var join = mars34.channels.get("806223592647688202"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars35.on("ready", () => {
    var join = mars35.channels.get("806223624888385546"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars36.on("ready", () => {
    var join = mars36.channels.get("806223667696107562"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars37.on("ready", () => {
    var join = mars37.channels.get("806223835472330823"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars38.on("ready", () => {
    var join = mars38.channels.get("806223874210791444"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars39.on("ready", () => {
    var join = mars39.channels.get("806223908281384990"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars40.on("ready", () => {
    var join = mars40.channels.get("806223957241888769"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars41.on("ready", () => {
    var join = mars41.channels.get("806224015139143710"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars42.on("ready", () => {
    var join = mars42.channels.get("806224082970345472"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars43.on("ready", () => {
    var join = mars43.channels.get("806224128000131142"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars44.on("ready", () => {
    var join = mars44.channels.get("806224157196419113"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars45.on("ready", () => {
    var join = mars45.channels.get("806224189403955260"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars46.on("ready", () => {
    var join = mars46.channels.get("806224229832982638"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars47.on("ready", () => {
    var join = mars47.channels.get("806224269671268352"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars48.on("ready", () => {
    var join = mars48.channels.get("806224314944192523"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars49.on("ready", () => {
    var join = mars49.channels.get("806224351548538960"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars50.on("ready", () => {
    var join = mars50.channels.get("806224384837943296"); /// ئاید ڤۆیس
    if (join) join.join();
  });
 mars51.on("ready", () => {
    var join = mars51.channels.get("806167974649397280"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars52.on("ready", () => {
    var join = mars52.channels.get("806168050142543932"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars53.on("ready", () => {
    var join = mars53.channels.get("806168157533634643"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars54.on("ready", () => {
    var join = mars54.channels.get("806168190957781043"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars55.on("ready", () => {
    var join = mars55.channels.get("806168236017582151"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars56.on("ready", () => {
    var join = mars56.channels.get("806168312676483153"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars57.on("ready", () => {
    var join = mars57.channels.get("806168350752243734"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars58.on("ready", () => {
    var join = mars58.channels.get("806168389818253365"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars59.on("ready", () => {
    var join = mars59.channels.get("806168442327007238"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars60.on("ready", () => {
    var join = mars60.channels.get("806168550389055508"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars61.on("ready", () => {
    var join = mars61.channels.get("806203511623647353"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars62.on("ready", () => {
    var join = mars62.channels.get("806222675302940713"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars63.on("ready", () => {
    var join = mars63.channels.get("806222716100280371"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars64.on("ready", () => {
    var join = mars64.channels.get("806222753224327228"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars65.on("ready", () => {
    var join = mars65.channels.get("806222820094509146"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars66.on("ready", () => {
    var join = mars66.channels.get("806222940563308604"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars67.on("ready", () => {
    var join = mars67.channels.get("806222970791526432"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars68.on("ready", () => {
    var join = mars68.channels.get("806223002382762024"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars69.on("ready", () => {
    var join = mars69.channels.get("806223034678509629"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars70.on("ready", () => {
    var join = mars70.channels.get("806223066064486489"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars71.on("ready", () => {
    var join = mars71.channels.get("806223100529999933"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars72.on("ready", () => {
    var join = mars72.channels.get("806223176560410670"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars73.on("ready", () => {
    var join = mars73.channels.get("806223205639520297"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars74.on("ready", () => {
    var join = mars74.channels.get("806223235497721886"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars75.on("ready", () => {
    var join = mars75.channels.get("806223269433966643"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars76.on("ready", () => {
    var join = mars76.channels.get("806223298979299368"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars77.on("ready", () => {
    var join = mars77.channels.get("806223350262530150"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars78.on("ready", () => {
    var join = mars78.channels.get("806223383532142662"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars79.on("ready", () => {
    var join = mars79.channels.get("806223414389243945"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars80.on("ready", () => {
    var join = mars80.channels.get("806223468969459722"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars81.on("ready", () => {
    var join = mars81.channels.get("806223501283426325"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars82.on("ready", () => {
    var join = mars82.channels.get("806223533440237639"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars83.on("ready", () => {
    var join = mars83.channels.get("806223561944727584"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars84.on("ready", () => {
    var join = mars84.channels.get("806223592647688202"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars85.on("ready", () => {
    var join = mars85.channels.get("806223624888385546"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  ////////////////////////////////////
  mars86.on("ready", () => {
    var join = mars86.channels.get("806223667696107562"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars87.on("ready", () => {
    var join = mars87.channels.get("806223835472330823"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars88.on("ready", () => {
    var join = mars88.channels.get("806223874210791444"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars89.on("ready", () => {
    var join = mars89.channels.get("806223908281384990"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars90.on("ready", () => {
    var join = mars90.channels.get("806223957241888769"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars91.on("ready", () => {
    var join = mars91.channels.get("806224015139143710"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars92.on("ready", () => {
    var join = mars92.channels.get("806224082970345472"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars93.on("ready", () => {
    var join = mars93.channels.get("806224128000131142"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars94.on("ready", () => {
    var join = mars94.channels.get("806224157196419113"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars95.on("ready", () => {
    var join = mars95.channels.get("806224189403955260"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars96.on("ready", () => {
    var join = mars96.channels.get("806224229832982638"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars97.on("ready", () => {
    var join = mars97.channels.get("806224269671268352"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars98.on("ready", () => {
    var join = mars98.channels.get("806224314944192523"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars99.on("ready", () => {
    var join = mars99.channels.get("806224351548538960"); /// ئاید ڤۆیس
    if (join) join.join();
  });
  mars100.on("ready", () => {
    var join = mars100.channels.get("806224384837943296"); /// ئاید ڤۆیس
    if (join) join.join();
  });
////////////////////////////////
mars.login(process.env.m1);
mars2.login(process.env.m2);
mars3.login(process.env.m3);
mars4.login(process.env.m4);
mars5.login(process.env.m5);
mars6.login(process.env.m6);
mars7.login(process.env.m7);
mars8.login(process.env.m8);
mars9.login(process.env.m9);
mars10.login(process.env.m10);
mars11.login(process.env.m11);
mars12.login(process.env.m12);
mars13.login(process.env.m13);
mars14.login(process.env.m14);
mars15.login(process.env.m15);
mars16.login(process.env.m16);
mars17.login(process.env.m17);
mars18.login(process.env.m18);
mars19.login(process.env.m19);
mars20.login(process.env.m20);
mars21.login(process.env.m21);
mars22.login(process.env.m22);
mars23.login(process.env.m23);
mars24.login(process.env.m24);
mars25.login(process.env.m25);
mars26.login(process.env.m26);
mars27.login(process.env.m27);
mars28.login(process.env.m28);
mars29.login(process.env.m29);
mars30.login(process.env.m30);
mars31.login(process.env.m31);
mars32.login(process.env.m32);
mars33.login(process.env.m33);
mars34.login(process.env.m34);
mars35.login(process.env.m35);
mars36.login(process.env.m36);
mars37.login(process.env.m37);
mars38.login(process.env.m38);
mars39.login(process.env.m39);
mars40.login(process.env.m40);
mars41.login(process.env.m41);
mars42.login(process.env.m42);
mars43.login(process.env.m43);
mars44.login(process.env.m44);
mars45.login(process.env.m45);
mars46.login(process.env.m46);
mars47.login(process.env.m47);
mars48.login(process.env.m48);
mars49.login(process.env.m49);
mars50.login(process.env.m50);
mars51.login(process.env.m51);
mars52.login(process.env.m52);
mars53.login(process.env.m53);
mars54.login(process.env.m54);
mars55.login(process.env.m55);
mars56.login(process.env.m56);
mars57.login(process.env.m57);
mars58.login(process.env.m58);
mars59.login(process.env.m59);
mars60.login(process.env.m60);
mars61.login(process.env.m61);
mars62.login(process.env.m62);
mars63.login(process.env.m63);
mars64.login(process.env.m64);
mars65.login(process.env.m65);
mars66.login(process.env.m66);
mars67.login(process.env.m67);
mars68.login(process.env.m68);
mars69.login(process.env.m69);
mars70.login(process.env.m70);
mars71.login(process.env.m71);
mars72.login(process.env.m72);
mars73.login(process.env.m73);
mars74.login(process.env.m74);
mars75.login(process.env.m75);
mars76.login(process.env.m76);
mars77.login(process.env.m77);
mars78.login(process.env.m78);
mars79.login(process.env.m79);
mars80.login(process.env.m80);
mars81.login(process.env.m81);
mars82.login(process.env.m82);
mars83.login(process.env.m83);
mars84.login(process.env.m84);
mars85.login(process.env.m85);
mars86.login(process.env.m86);
mars87.login(process.env.m87);
mars88.login(process.env.m88);
mars89.login(process.env.m89);
mars90.login(process.env.m90);
mars91.login(process.env.m91);
mars92.login(process.env.m92);
mars93.login(process.env.m93);
mars94.login(process.env.m94);
mars95.login(process.env.m95);
mars96.login(process.env.m96);
mars97.login(process.env.m97);
mars98.login(process.env.m98);
mars99.login(process.env.m99);
mars100.login(process.env.m100);
//////////////////////////////////
